...
export default function StudentDashboard() {
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => api.getDashboard('student'),
  })
    ...
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm border border-gray-200 dark:border-gray-700">
          <h2 className="font-semibold text-gray-900 dark:text-white mb-4">Today's Classes</h2>
          <div className="space-y-3">
            {data?.today_classes?.map((classItem, i) => (
            ...
            {data?.upcoming_assignments?.map((assignment) => (
              <div key={assignment.id} className="p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                  Due: {assignment.deadline} • {assignment.subject}
                </p>
              </div>
            ))}
          </div>
